...
@Stateful
@RequestScoped
public class ManageGroupEventsHome implements Serializable 
{
	@Inject
	...
	private List<Event> assignableEvents;
	...
	public void init()
	{	
		this.assignableEvents = es.findAll();
		this.assignedEvents = es.findAssignedEvents(group);	
	}
	...
	public List<Event> getAssignableEvents() {
		return assignableEvents;
	}

	public void setAssignableEvents(List<Event> assignableEvents) {
		this.assignableEvents = assignableEvents;
	}
	...
	public String save()
	{
		...
		try
		{
			es.removeAllEventsByGroup(group);
		
			for ( Event event : assignedEvents )
			{
				es.addEventToGroup( group, event );
			}
