...
public class EmailValidator implements Validator
{
	...
	public void validate(FacesContext context, UIComponent component, Object value) throws ValidatorException
	{
		if ( value != null )
		{
			String emails = value.toString().trim();
			
			if ( !StringUtils.isEmpty( emails ) )
			{
				log.debugv( "Validating email: {0}", emails );
				
				for ( String email : emails.split( SystemConstant.EMAIL_SEPARATOR ) )
				{
					matcher = pattern.matcher(email.trim());
					if( !matcher.matches() )
					{
						log.debugv( "Invalid email: {0}", email );
						String text = bundleManager.getMessage( "validator.bademail", email );
